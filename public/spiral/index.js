...
for (let h = 0; h < appHeight; h += textureSize) {
  for (let w = 0; w < appHeight; w += textureSize) {
    let background = new PIXI.Sprite.from("canvas_texture.png");
...
const coordinateFinder = (index) => {
  let angle = (((360 / circlePerLoop / 2 + 360) / circlePerLoop) * index) % 360;
  let renderDistance = Math.min(maxRenderDistance, frontIndex);
  let radius =
    r + ((R - r) / renderDistance) * (index - (frontIndex - renderDistance));
...
for (let i = 0; i < totalCircleCount; i++) {
  let circle = new PIXI.Sprite.from("particle.gif");
...
app.ticker.add((delta) => {
  ...
  for (let i = 0; i < totalCircleCount; i++) {
    ...
    if (Circles[i].type == "instant" || frontIndex - i <= circlePerLoop * 5) {
