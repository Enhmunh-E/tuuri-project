...
const PixiComponent = () => {
    ...
    useEffect(() => {
        ...
        const coordinateFinder = (index) => {
            let angle =
                (((360 / circlePerLoop / 2 + 360) / circlePerLoop) * index) %
                360;
            ...
            let radius =
                ...
                ((R - r) / renderDistance) *
                    (index - (frontIndex - renderDistance));
            let x =
                radius * Math.cos((angle * Math.PI) / 180) +
                rX +
                ((Rx - rX) / renderDistance) *
                    (index - (frontIndex - renderDistance));
            let y =
                radius * Math.sin((angle * Math.PI) / 180) +
                rY +
                ((Ry - rY) / renderDistance) *
                    (index - (frontIndex - renderDistance));
            return { x: x, y: y };
        };
        ...
        app.ticker.add((delta) => {
            ...
            let howMany = Math.floor(window.innerWidth / 256) + 2;
            howMany = howMany % 2 == 0 ? howMany + 1 : howMany;

            resIndex = Math.floor(
                ((document.getElementById("list")?.scrollLeft / (256 + 16)) *
            ...
            if (frontIndex == resIndex && pixiSizeCorrect && visibleState) {
                ...
                if (popup.inUse || !circleSizeCorrect) {
                    ...
                    if (
                        Math.abs(Circles[frontIndex].circle.height - psize) <
                        popup.delay
                    ) {
